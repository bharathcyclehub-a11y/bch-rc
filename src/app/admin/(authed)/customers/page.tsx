...
type ListState = { q: string; segment: Segment; sort: Sort };
...
export default async function AdminCustomers({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [rows, [counts]] = await Promise.all([
    db
      ...
      .groupBy(customers.id)
      .having(segmentHaving(state.segment, { orderCount: ORDER_COUNT, ltv: LTV, lastOrderAt: LAST_ORDER }, inactiveCutoffIso))
  ...
  const description = state.q
    ...
    : `${plural(counts?.all ?? 0, "customer")} · ${plural(counts?.repeat ?? 0, "repeat buyer")} · ${SORT_PHRASE[state.sort]}`;
  ...
  return (
    <>
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        />
        <UrlFilters filters={[SORT_FILTER]} />
      </div>
